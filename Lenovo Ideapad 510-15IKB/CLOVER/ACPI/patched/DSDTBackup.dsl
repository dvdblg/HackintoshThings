 ...
 *
 * Disassembly of /Volumes/EFI/EFI/CLOVER/ACPI/patched/DSDT.aml, Fri Dec  1 20:09:28 2017
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00018867 (100455)
 *     Revision         0x02
 *     Checksum         0xAC
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
    ...
    External (_PR_.HWPI, FieldUnitObj)    // (from opcode)
    ...
    Method (GSKU, 0, Serialized)
    {
        ...
        Return (LAnd (LEqual (GPD9, One), LEqual (GPDA, Zero)))
    }
    ...
    Scope (_SB.PCI0.I2C1)
    {
        Device (TPL1)
        {
            ...
            CreateWordField (SBFB, \_SB.PCI0.I2C1.TPL1._Y21._ADR, BADR)  // _ADR: Address
            CreateDWordField (SBFB, \_SB.PCI0.I2C1.TPL1._Y21._SPE, SPED)  // _SPE: Speed
            CreateDWordField (SBFI, \_SB.PCI0.I2C1.TPL1._Y22._INT, INT2)  // _INT: Interrupts
            CreateWordField (SBFG, 0x17, INT1)
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                If (LLess (OSYS, 0x07DC))
        ...
        Device (BTH0)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                ...
                Name (SBFI, ResourceTemplate ()
                {
                    ...
                    Interrupt (ResourceConsumer, Edge, ActiveLow, Exclusive, ,, _Y26)
                    {
                        0x00000000,
                    }
                })
            ...
            Device (BAT0)
            {
                ...
                Method (_BST, 0, Serialized)  // _BST: Battery Status
                {
                    ...
                    If (LEqual (ECON, One))
                    {
                        ...
                        Sleep (0x10)
                        Store (0x32, Local3)
                        Store (LRAM (0x04, 0xC2), Local4)
                        Store (LRAM (0x04, 0xC3), Local5)
                        Or (ShiftLeft (Local5, 0x08), Local4, OBRC)
                        If (LEqual (OBRC, Zero))
                        {
                            WECM (0x30, 0x33)
                            While (Local3)
    ...
    Method (_WAK, 1, Serialized)  // _WAK: Wake
    {
        ...
        Store (\_SB.PCI0.LPCB.PKBD, Local2)
        Store (\_SB.PCI0.LPCB.ECBD, Local2)
        ADBG ("_WAK")
        \_SB.PCI0.GEXP.INVC ()
        If (LEqual (S0ID, One))
        {
    ...
    Scope (_SB)
    {
        ...
        Device (EPC)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                CreateQWordField (RBUF, \_SB.EPC._Y28._MIN, EMIN)  // _MIN: Minimum Base Address
                CreateQWordField (RBUF, \_SB.EPC._Y28._MAX, EMAX)  // _MAX: Maximum Base Address
                CreateQWordField (RBUF, \_SB.EPC._Y28._LEN, ELEN)  // _LEN: Length
                Store (\_PR.EMNA, EMIN)
                Store (\_PR.ELNG, ELEN)
    ...
    Scope (_SB.PCI0)
    {
        Device (PDRC)
        {
            ...
            Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
            {
                ...
                Store (GPCL (), XSZ0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y2D._BAS, SXRA)  // _BAS: Base Address
                Store (SXRB, SXRA)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y2D._LEN, SXRL)  // _LEN: Length
                Store (SXRS, SXRL)
                If (LNot (HPTE))
                {
                    CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y2E._BAS, HBAS)  // _BAS: Base Address
                    CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y2E._LEN, HLEN)  // _LEN: Length
                    Store (HPTB, HBAS)
                    Store (0x0400, HLEN)
                }
